...
#
# 2026-08-16 修正: 5分刻みだったため、TAKAツールの実打刻（26:44・25:32 等）が
# 選択肢に無く、一覧の該当セルが**空欄に見えていた**（データは入っているのに
# 「反映されていない」と誤認され、入力し直すと正確な打刻が5分刻みに丸められる
# 事故につながる）。支払いは1分単位で計算しているので、選択肢も1分刻みにする。
TIME_OPTIONS: list = (
    [EMPTY_TIME]
    + [f"{h:02d}:{m:02d}"
