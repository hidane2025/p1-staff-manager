...
GitHub Actions で push のたびに **13スイート・215+件** のテストを自動実行。  
...
| 18 | 10 | ページ起動スモーク（AppTest） |
| 19 | 10 | admin_guard（PWゲート） |
| 20 | 60+ | core logic（計算/紙幣/地域/シフトパース） |
| 21 | 50+ | UI要素検出（AppTest 主要ページ） |
| 22 | 30 | セキュリティ動作（HTMLエスケープ・要件） |
| **23** | **8** | **スタッフ名寄せ（NO.>email>name_jp / NFKC正規化）** |
| **24** | **8** | **弁当配布チェック（lunch_status 3状態・正規化）** |
...
| 管理者ゲート | PII を扱う16ページに `require_admin()`（多ユーザーID/PASS＋ロール認証、または `ADMIN_PASSWORD` Secret） |
| 監査ログ | PII閲覧・ログイン成功/失敗を `p1_audit_log` に記録 |
| Storage RLS | receipts/contracts バケットは public OFF。**アプリがトークン検証 → server-side でPDF bytes 配信**（Signed URL は旧経路に一部残存） |
| HTMLエスケープ | スタッフ名等のDB値を画面表示（封筒明細）・契約書PDFで `html.escape()` |
