...
# 内訳表の「行の合計＝支給合計」不変条件（2026-08-06 中野さん指摘）
#   休憩控除（基本給に反映済み）をマイナス行で二重表示し、引き忘れに見えた。
#   個別手当はDBに無い列を参照し、手当があっても行が出なかった。
print("\n[13d] 内訳表の整合表示")
_check("休憩控除のマイナス行を表に出さない（注記に移動）",
       "| 休憩控除 | -¥" not in _pay,
       "控除済みの額をマイナス行で並べると行の合計が総額と合わない")
_check("基本給ラベルに控除後と明記", "休憩控除後" in _pay)
