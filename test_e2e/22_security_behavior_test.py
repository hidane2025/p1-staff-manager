...
# ============================================================
# 13. 交通費の上限解釈が3画面で一致しているか（2026-08-02）
#     上限の意味が「総額」と「日額×勤務日数」で分裂し、同じ人が
#     入力画面によって最大¥15,000変わっていた。画面表記（上限額/人）と
#     実データ（東北¥15,000＝往復総額）から「総額」に統一した。
# ============================================================
print("\n[13] 交通費の上限解釈の一致")
...
_check("ピット端末が上限を日数倍していない",
       "max_amt * days_worked" not in _pit,
       "上限×勤務日数は交通費ページ（総額）と食い違う")
_check("支払い計算の開催地一律が日数倍していない",
       'or 0) * days, "開催地一律"' not in _pay,
       "事前見積（上限額/人）の日数倍が支給され、現金準備が不足する")
_check("交通費ページは総額として上限を適用している",
