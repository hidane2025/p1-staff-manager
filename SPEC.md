...
| 主要機能の搭載状態 | 多ユーザー認証 / 応募管理 / SUPABASE_SERVICE_KEY 対応 / スタッフ名寄せ / PII ガード16ページ — **すべて搭載済**（コミットメッセージ上は v番号未付与だが、`utils/admin_guard.py:1` のコメントには `v3.15 多ユーザー認証` の表記あり） |
...
| 12 | `12_応募管理.py` | 応募管理 | GoogleフォームDB連携の採用/却下 | ✅ | あり |
| 13 | `13_応募フォーム設定.py` | 応募フォーム設定 | スプレッドシート ID 登録 | ✅ | なし |
...
| `p1_dealer_applications` | ディーラー応募（Googleフォーム取込） | **高**（本名・連絡先） |
| `p1_application_sources` | 応募スプレッドシート設定 | 低 |
...
- UI は `pages/10_ピット端末.py` に「📦 弁当配布チェック」エキスパンダーで内蔵（タブレット運用想定）
