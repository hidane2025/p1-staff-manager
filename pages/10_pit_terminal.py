        ...
        if st.form_submit_button("💾 交通費を保存", type="secondary"):
            ...
            db.log_action(
                ...
                detail=(
                    ...
                    f"→ 精算¥{approved:,}"
                    f"（勤務{days_worked}日・"
                    f"{'開催地一律 日額×勤務日数' if is_venue else '領収書ベース・上限は往復総額'}）"
                ),
                event_id=event_id,
                performed_by=operator_name(),
            )
            st.success(
                ...
                f"（{'開催地一律（日額×勤務日数）' if is_venue else '領収書ベース／上限は往復総額で判定'}）"
            )
            st.rerun()
...
if not today_shift:
    ...
    # 2026-08-13: 途中離脱ガード。同日夜に方針変更があり、未打刻の予定日は
    # 金額に入らなくなった（過払いの危険は解消）。ただし予定シフトを残したまま
    # 清算すると翌日以降の出欠一覧に「未確定」で残り続けるため、
    # 記録を締める導線として欠勤化ボタンは維持する。
    ...
    if _future:
        ...
        st.warning(
            ...
            "予定日の金額は打刻するまで¥0なので支払いに影響はありませんが、"
            "今日で最後（途中離脱）なら残り日程を欠勤にして記録を締めてください。"
        )
